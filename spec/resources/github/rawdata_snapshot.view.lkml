view: rawdata_snapshot {
  derived_table: {
    sql:
      SELECT rawdata.*, TIMESTAMP(snapshot.snapshot_date) as snapshot_date
      FROM ${rawdata_date_array.SQL_TABLE_NAME} snapshot
      CROSS JOIN ${rawdata.SQL_TABLE_NAME} rawdata
      WHERE TIMESTAMP(rawdata.ts_action) <= TIMESTAMP(snapshot.snapshot_date);;
  }
